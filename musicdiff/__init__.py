...
def diff_ml_training(
    ...
    for name in os.listdir(ground_truth_folder):
        gtpath = os.path.join(ground_truth_folder, name)
        if not os.path.isfile(gtpath):
            continue
        stem = Path(name).stem
        gt_by_stem.setdefault(stem, []).append(name)

    num_pred_files: int = 0
    num_matched_pairs: int = 0
    num_missing_gt: int = 0
    num_ambiguous_gt: int = 0

    num_comparison_errors: int = 0
    ...
    for name in os.listdir(predicted_folder):
        ...
        # First try exact filename, then fallback to stem match (xml <-> krn, etc).
        gtpath: str = os.path.join(ground_truth_folder, name)
        if not os.path.isfile(gtpath):
            stem_matches = gt_by_stem.get(Path(name).stem, [])
            if len(stem_matches) == 1:
                gtpath = os.path.join(ground_truth_folder, stem_matches[0])
        ...
        try:
            ...
            print(
                f'Comparison failed for pair {name}: {type(e).__name__}: {e}',
                file=sys.stderr
            )
            num_comparison_errors += 1
            continue

    if num_matched_pairs == 0:
        print(
            'diff_ml_training: no comparable file pairs found. '
            + f'pred files={num_pred_files}, missing gt={num_missing_gt}, '
            + f'ambiguous gt stem matches={num_ambiguous_gt}, '
            + f'comparison errors={num_comparison_errors}.',
            file=sys.stderr
        )
